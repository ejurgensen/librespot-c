...
// The intended use of this function is to decode a Spotify web api access token
// (194 bytes) to the binary form that the protobuf interface requires. The
// function outputs 146 bytes for this token, which is the same as the stored
// credentials, so even though the token isn't working the decoding here might
...
crypto_base64_to_bin(uint8_t *out, size_t out_len, const char *in)
{
  ...
  for (ptr = in; *ptr; ptr++)
    {
      // n = 64 * n + base64_digit(*p);
      bnum_mul(n, n, base);
      u8 = crypto_base64_digit(*ptr);
